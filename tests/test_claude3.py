...
def test_args():
    ...
    assert b == '{"messages": [{"role": "user", "content": "A"}], "max_tokens": 500, "stop_sequences": [], "temperature": 0.5, "top_p": 1, "anthropic_version": "bedrock-2023-05-31"}'
...
def test_args_add_stop_seq():
    ...
    assert b == '{"messages": [{"role": "user", "content": "A"}], "max_tokens": 500, "stop_sequences": ["Hello"], "temperature": 0.5, "top_p": 1, "anthropic_version": "bedrock-2023-05-31"}'
    
def test_args_topk():
    b = fm.get_body(prompt="A", top_p=1, temperature=0.5, max_token_count=500, stop_sequences=[], extra_args={"top_k": 200}, stream=False)
    assert b == '{"top_k": 200, "messages": [{"role": "user", "content": "A"}], "max_tokens": 500, "stop_sequences": [], "temperature": 0.5, "top_p": 1, "anthropic_version": "bedrock-2023-05-31"}'
...
def test_generate_details():
    ...
    assert r.body == '{"messages": [{"role": "user", "content": "test"}], "max_tokens": 500, "stop_sequences": [], "temperature": 0.7, "top_p": 1, "anthropic_version": "bedrock-2023-05-31"}'
    assert r.latency > 0
...
def test_chat_streaming():
    r = fm.chat([System("You are a helpful assistant"), Human("What is your name?")], stream=True)
    for x in r:
